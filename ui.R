...
suppressWarnings(suppressMessages(library(shinycssloaders, quietly = T, warn.conflicts = F)))
...
body <- dashboardBody(
  fluidRow(
    column(8, 
           ...
           box(width = NULL, solidHeader = TRUE, status = "primary", title = "7 Day Forecast",
           ...
           tabBox(width = NULL, height = 600,
             tabPanel("Weather Data", 
                      withSpinner(leafletOutput("mapPlot", height = 500))
                      ),
    ...
    column(4, 
           ...
           box(width = NULL, status = "warning",
               ...
               sliderInput("timeval", "Time", value = ceiling_date(with_tz(Sys.time(), "America/New_York"), unit = "hour"), min = ceiling_date(with_tz(Sys.time(), "America/New_York"), unit = "hour"),
                           max = ceiling_date(with_tz(Sys.time(), "America/New_York"), unit = "hour") + hours(71), step = 3600,
  ...
  fluidRow(
    ...
    column(6, 
           box(width = NULL, solidHeader = TRUE, status = "warning",
               title = "Winds",
               htmlOutput("windWarningText")
               ) ,
           box(width = NULL, solidHeader = TRUE, status = "warning",
               title = "Coastal",
               htmlOutput("coastalWarningText")
           )
    )
  )
)
  ...
  #               # sliderInput("windtime", "Time", value = ceiling_date(with_tz(Sys.time(), "America/New_York"), unit = "hour"), min = ceiling_date(with_tz(Sys.time(), "America/New_York"), unit = "hour"),
  #               #             max = ceiling_date(with_tz(Sys.time(), "America/New_York"), unit = "hour") + hours(47), step = 3600,
  #               #             animate = T, timezone = NULL)
  #               
  #           )
  #   ),
